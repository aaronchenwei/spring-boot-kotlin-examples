package io.github.aaronchenwei.springboot.example.user
      ...
      criterias.contains("email") -> {
        ...
        if (criteriaValue.isNullOrBlank()) {
          ...
          ServerResponse
            .ok()
            .json()
            .bodyAndAwait(service.findByEmail(criteriaValue))
        }
      }
      else ->
        ServerResponse
  ...
  suspend fun findUser(request: ServerRequest): ServerResponse {
    ...
    return if (id == null) {
      ServerResponse
        .badRequest()
        .json()
        .bodyValueAndAwait(ErrorMessage("`id` must be numeric"))
    } else {
  ...
  suspend fun addUser(request: ServerRequest): ServerResponse {
    ...
    return if (newUser == null) {
      ServerResponse
        .badRequest()
        .json()
        .bodyValueAndAwait(ErrorMessage("Invalid body"))
    } else {
